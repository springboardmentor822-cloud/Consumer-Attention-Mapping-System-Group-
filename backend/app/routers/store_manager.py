...
SM_ROLES = (UserRole.STORE_MANAGER, UserRole.ADMINISTRATOR)


@router.get("/dashboard")
@router.get("/stores/{store_id}/dashboard")
def get_sm_dashboard(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_dashboard_data(db, store_id)


@router.get("/live-cameras")
@router.get("/stores/{store_id}/live-cameras")
def get_sm_live_cameras(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_cameras_data(db, store_id)


@router.get("/visitors")
@router.get("/stores/{store_id}/visitors")
def get_sm_visitors(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_visitors_data(db, store_id)


@router.get("/store-traffic")
@router.get("/stores/{store_id}/store-traffic")
def get_sm_traffic(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
...
def get_sm_shelf(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
...
def get_sm_product(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_product_data(db, store_id)


@router.get("/heatmaps")
@router.get("/stores/{store_id}/heatmaps")
def get_sm_heatmaps(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_heatmaps_data(db, store_id)


@router.get("/alerts")
@router.get("/stores/{store_id}/alerts")
def get_sm_alerts(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_alerts_data(db, store_id)


@router.get("/reports")
@router.get("/stores/{store_id}/reports")
def get_sm_reports(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_reports_data(db, store_id)


@router.get("/activities")
@router.get("/stores/{store_id}/activities")
def get_sm_activities(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
    return get_store_manager_activities_data(db, store_id)


@router.get("/settings")
@router.get("/stores/{store_id}/settings")
def get_sm_settings(store_id: int = 1, db: Session = Depends(get_db), _=Depends(require_role(*SM_ROLES))):
