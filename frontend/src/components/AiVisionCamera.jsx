  ...
  useEffect(() => {
    async function loadAiModel() {
      try {
        ...
        setStatus("YOLOv8 Active");
      } catch (err) {
        console.error("TensorFlow load error", err);
  ...
  return (
    <div className="bg-[#111827] border border-[#273449] rounded-xl p-2.5 space-y-2 font-sans">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[9px] font-mono bg-blue-500/10 border border-blue-500/30 text-blue-400 px-2 py-0.5 rounded font-bold">
          {status}
        </span>
      </div>
      ...
      <div className="relative w-full h-36 bg-[#000000] border border-[#273449] rounded-lg overflow-hidden flex items-center justify-center">
        <video
          ...
          style={{ filter: "contrast(105%) saturate(90%) hue-rotate(-10deg)", mixBlendMode: "normal" }}
        />
        <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none w-full h-full object-cover" />
      </div>

      <div className="flex items-center space-x-1.5 pt-0.5">
        <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
        <span className="text-[10px] text-slate-300 font-extrabold">● Live Telemetry Stream</span>
      </div>
    </div>
  );
}