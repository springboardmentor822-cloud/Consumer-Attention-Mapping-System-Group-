...
import {
  ...
  journeyFunnel, sparklines, heatColor, formatNumber, formatCurrency
...
const t = trafficOverview;
const topProducts = products.slice().sort((a, b) => b.attentionScore - a.attentionScore).slice(0, 5);
const segPie = customerSegments.map(s => ({ name: s.name, value: s.pct, color: s.color }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs">Comprehensive intelligence hub — shopper activity, attention metrics, sales performance & AI insights.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
        <KPI label="Total Visitors" value={formatNumber(t.totalVisitors)} change={`↑ ${t.totalVisitorsChange}%`} changeColor="text-emerald-400" sparkData={sparklines.visitors} sparkColor="#06B6D4" sub="vs last 7 days" />
        <KPI label="Avg Attention Time" value={`${t.avgAttentionTime}s`} change={`↑ ${t.avgAttentionTimeChange}%`} changeColor="text-emerald-400" sparkData={sparklines.attention} sparkColor="#8B5CF6" sub="gaze fixation" />
        <KPI label="Avg Dwell Time" value={`${t.avgDwellTime} min`} change={`↑ ${t.avgDwellTimeChange}%`} changeColor="text-emerald-400" sparkData={sparklines.dwell} sparkColor="#10B981" sub="per visitor" />
        <KPI label="Conversion Rate" value={`${t.conversionRate}%`} change={`↑ ${t.conversionRateChange}%`} changeColor="text-emerald-400" sparkData={sparklines.conversion} sparkColor="#F59E0B" sub="browse-to-purchase" />
        <KPI label="Sales Revenue" value={formatCurrency(t.salesRevenue)} change={`↑ ${t.salesRevenueChange}%`} changeColor="text-emerald-400" sparkData={sparklines.revenue} sparkColor="#EC4899" sub="total period" />
        <KPI label="Avg Order Value" value={`$${t.avgOrderValue}`} change={`↑ ${t.avgOrderValueChange}%`} changeColor="text-emerald-400" sparkData={sparklines.aov} sparkColor="#06B6D4" sub="per transaction" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-[#0F172A] border border-[#1E293B] p-5 rounded-2xl space-y-3">
          ...
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={shoppingBehavior} layout="vertical">
                <CartesianGrid stroke="#1E293B" strokeDasharray="3 3" />
      ...
      <div className="bg-[#0F172A] border border-[#1E293B] p-5 rounded-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {aiInsights.slice(0, 4).map((rec, i) => (
